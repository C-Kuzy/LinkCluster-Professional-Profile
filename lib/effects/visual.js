...
for (let i = 0; i < windStreakCount; i++) {
    ...
    // Evenly distribute particles from off-screen left to on-screen
    // This prevents initial clumping and makes them flow in naturally
    const leftBoundary = -(breezeConfig.areaWidth / 2);
    const rightBoundary = (breezeConfig.areaWidth / 2);
    const totalSpread = (rightBoundary - leftBoundary) + 150; // Extra space for off-screen positioning
    const evenSpacing = (i / windStreakCount) * totalSpread;
    
    for (let j = 0; j < positions.length; j += 3) {
        positions[j] = leftBoundary - 100 + evenSpacing; // Start from far left off-screen
    }
